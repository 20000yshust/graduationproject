...
parser.add_argument("--prune-layer", type=str, default="layer3")
parser.add_argument("--prune-rate", type=str, default="np.linspace(0, 1, 20, endpoint=False)")
parser.add_argument("--checkpoint-path", type=str, default="/data/yangsheng/Myinfectedmodel.pth.tar")
parser.add_argument("--outfile", type=str, default="CIFAR10_ResNet-18_BadNets_pruned_666.txt")
#Device options
...
def get_pruned_model(model, tr_loader, prune_rate, layer_to_prune):
    ...
    with torch.no_grad():
        ...
        hook2=register_forwardhook_for_resnet(model,"resnet18",1,"layer2.1.shortcut")
        hook3 = register_forwardhook_for_resnet(model, "resnet18", 0.1, "layer3.1.shortcut")
...
with open(args.outfile, "w+") as outs:
    ...
    for pruning_rate in eval(args.prune_rate):
        ...
        k=[]
        k.append(pruning_rate)
        k.append(BA.item())
        k.append(ASR.item())
        data.append(k)
    import csv
    with open("pruning_reweight0.1_data_global_seed666_blended_layer3.csv", "a", newline="") as f:
        writer = csv.writer(f)
